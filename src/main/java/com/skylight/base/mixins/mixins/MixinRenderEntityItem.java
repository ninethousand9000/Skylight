...
import com.skylight.client.modules.visual.ItemPhysics;
...
public abstract class MixinRenderEntityItem extends MixinRender<EntityItem> {
    ...
    {
        if (Skylight.MODULE_MANAGER.getModule(ItemPhysics.class).isEnabled())
        {
            ...
            if (itemstack.getItem() != null) {
                ...
                for (int j = 0; j < i; ++j) {
                    if (ibakedmodel.isAmbientOcclusion()) {
                        GlStateManager.pushMatrix();
                        GlStateManager.scale(ItemPhysics.itemScale.getValue(), ItemPhysics.itemScale.getValue(), ItemPhysics.itemScale.getValue());
